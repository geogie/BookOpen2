package com.george.bookopen.book2;
 ...
 * Description:
 */
public class BookOpenView2 extends FrameLayout implements Animator.AnimatorListener {
    ...
    private AnimatorSet mAnimatorSet3;// 书本平移动画
    ...
    public BookOpenView2(@NonNull Context context) {
        super(context, null, 0);
        initView(context);
    }

    public BookOpenView2(@NonNull Context context, @NonNull AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BookOpenView2(@NonNull Context context, @NonNull AttributeSet attrs, int defStyleAttr) {
     ...
     * 平移过度到目标位置---》书本打开效果
     *
     * @param activity   用于获取window大小
     * @param startValue
     * @param endValue
     */
    public synchronized void startAnimTran(final Activity activity, BookOpenViewValue startValue, final BookOpenViewValue endValue) {
        if (!isOpened.get()) {
            ViewGroup.LayoutParams layoutParams;
            mStartValueTran = startValue;
            Log.d(TAG, " \nmStartValueTran:" + mStartValueTran.toString() + "\nendValue:" + endValue.toString());
            layoutParams = page_cover.getLayoutParams();
            // 把startValue的width和height给page_cover
            layoutParams.width = mStartValueTran.getRight() - mStartValueTran.getLeft();
            layoutParams.height = mStartValueTran.getBottom() - mStartValueTran.getTop();
            ...
            page_cover.setTranslationX(mStartValueTran.getLeft());
            page_cover.setTranslationY(mStartValueTran.getTop());

            page_content.setTranslationX(mStartValueTran.getLeft());
            page_content.setTranslationY(mStartValueTran.getTop());
            // 平移
            ObjectAnimator translationX = ObjectAnimator.ofFloat(page_cover, "translationX", mStartValueTran.getLeft(), endValue.getLeft());
            translationX.setDuration(1000);
            ObjectAnimator translationY = ObjectAnimator.ofFloat(page_cover, "translationY", mStartValueTran.getTop(), endValue.getTop());
            translationY.setDuration(1000);

            ObjectAnimator translationX2 = ObjectAnimator.ofFloat(page_content, "translationX", mStartValueTran.getLeft(), endValue.getLeft());
            translationX2.setDuration(1000);
            ObjectAnimator translationY2 = ObjectAnimator.ofFloat(page_content, "translationY", mStartValueTran.getTop(), endValue.getTop());
            translationY2.setDuration(1000);

            mAnimatorSet3 = new AnimatorSet();
            mAnimatorSet3.playTogether(translationX, translationY, translationX2, translationY2);
            mAnimatorSet3.addListener(new Animator.AnimatorListener() {
                @Override
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    BookOpenViewValue endValue2 = new BookOpenViewValue(window.getLeft(),
                            window.getTop(),
                            window.getRight(),
                            window.getBottom());
                    startAnim(endValue, endValue2);
                }
    ...
    public synchronized void closeAnim2() {
        if (isOpened.get()) {
            setVisibility(VISIBLE);
            setAlpha(1.0f);
            ...
            mAnimatorSet2.addListener(new Animator.AnimatorListener() {
                ...
                @Override
                public void onAnimationEnd(Animator animation) {
                    endTran(mStartValue, mStartValueTran);
                }
    ...
    public void endTran(BookOpenViewValue startValue, BookOpenViewValue endValue) {
        ViewGroup.LayoutParams layoutParams;
        Log.d(TAG, " \nstartValue:" + startValue.toString() + "\nendValue:" + endValue.toString());
        layoutParams = page_cover.getLayoutParams();
        // 把startValue的width和height给page_cover
        ...
        page_content.setTranslationY(startValue.getTop());
        // 平移
        ObjectAnimator translationX = ObjectAnimator.ofFloat(page_cover, "translationX", startValue.getLeft(), endValue.getLeft());
        translationX.setDuration(1000);
        ObjectAnimator translationY = ObjectAnimator.ofFloat(page_cover, "translationY", startValue.getTop(), endValue.getTop());
        translationY.setDuration(1000);

        ObjectAnimator translationX2 = ObjectAnimator.ofFloat(page_content, "translationX", startValue.getLeft(), endValue.getLeft());
        translationX2.setDuration(1000);
        ObjectAnimator translationY2 = ObjectAnimator.ofFloat(page_content, "translationY", startValue.getTop(), endValue.getTop());
        translationY2.setDuration(1000);
